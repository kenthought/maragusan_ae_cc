...
from income.serializers import IncomeViewSerializer, IncomeWriteSerializer
...
class IncomeList(APIView):
    ...
    def format_account_number(self, id, account_number, user):
        try:
            # Create ledger code
            ledger_code = {"ledger": "Income", "module_id": id}
            ...
            if ledger_serializer.is_valid():
                ...
                income_serializer = IncomeWriteSerializer(income, data=data, partial=True)
                if income_serializer.is_valid():
                    income_serializer.save()
                    return income_serializer.data
    ...
    def get(self, request, format=None):
        ...
        serializer = IncomeViewSerializer(income, many=True)
        return Response(serializer.data)

    def post(self, request, format=None):
        serializer = IncomeWriteSerializer(data=request.data)
...
class IncomeDetail(APIView):
    ...
    def get(self, request, pk, format=None):
        ...
        serializer = IncomeViewSerializer(income)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        income = self.get_object(pk)
        serializer = IncomeWriteSerializer(income, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
