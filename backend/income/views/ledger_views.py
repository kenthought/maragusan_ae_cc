from income.models import Ledger
...
class LedgerList(APIView):
    ...
    def update_balance(self, ledger_data):
        balance = self.get_balance(ledger_data["income"], "Income")
...
class LedgerDetail(APIView):
    ...
    def get_object(self, income):
        try:
            return Ledger.objects.filter(income_id=income)
        except Ledger.DoesNotExist:
            raise Http404

    def get(self, request, income, format=None):
        ledger = self.get_object(income)
        serializer = LedgerViewSerializer(ledger, many=True)
        array = []
        count = 0
