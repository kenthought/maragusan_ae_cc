from receivables.models import SavingsLedger
from receivables.serializers import SavingsLedgerViewSerializer, SavingsLedgerWriteSerializer
...
class SavingsLedgerList(APIView):
    ...
    def update_balance(self, ledger_data):
        balance = self.get_balance(ledger_data["receivables"], "Savings")
    ...
    def get(self, request, format=None):
        ledger = SavingsLedger.objects.all()
        serializer = SavingsLedgerViewSerializer(ledger, many=True)
        return Response(serializer.data)

    def post(self, request, format=None):
        ledger_serializer = SavingsLedgerWriteSerializer(data=request.data)
...
class SavingsLedgerDetail(APIView):
    permissions_clases = [permissions.IsAuthenticated]

    def get_object(self, receivables):
        try:
            return SavingsLedger.objects.filter(receivables_id=receivables)
        except SavingsLedger.DoesNotExist:
            raise Http404
